# Emoney Data Extraction Service

A robust Flask-RESTX API service for extracting Emoney-Service data using DLT (Data Load Tool) and PostgreSQL. Features comprehensive Swagger documentation, Docker support, and production-ready deployment.

## Features

- **Emoney-Service API Integration**: Extracts data via Emoney-Service REST API
...
│   └── api_service.py          # Emoney-Service API client
│
├── models/                     # Data models
│   └── __init__.py
│
...
- Emoney-Service API Access Token/Credentials
...
1. **Clone the repository**:
   ...
   cd eMoney-service-extraction
   ```

2. **Set up environment variables**:
   ```bash
   cp .env.example .env
   # Edit .env with your Emoney-Service API credentials
   ```
...
2. **Set up environment variables**:
   ...
   export EMONEY-SERVICE_API_TOKEN="your-token-here"
   ```

3. **Start PostgreSQL** (using Docker):
   ```bash
   docker run -d --name eMoney_service_postgres \
     -e POSTGRES_DB=eMoney_service_data_dev \
...
curl -X POST http://localhost:4709/api/v1/scan/start \
  ...
  -d '{
    "config": {
      "scanId": "eMoney-service-scan-001",
      "organizationId": "org-12345",
      "type": ["data"],
      "auth": {
        "accessToken": "your-eMoney-service-token-here"
      },
      "filters": {
...
#### Check Scan Status
```bash
curl http://localhost:4709/api/v1/scan/eMoney-service-scan-001/status
```
...
| `DB_NAME` | Database name | `eMoney_service_data_dev` |
| `DB_USER` | Database user | `postgres` |
| `DB_PASSWORD` | Database password | `password123` |
| `DB_SCHEMA` | Database schema | `eMoney_service_dev` |
| `EMONEY-SERVICE_API_TOKEN` | Emoney-Service API token | `""` |
| `EMONEY-SERVICE_API_TIMEOUT` | API timeout seconds | `30` |
| `EMONEY-SERVICE_API_RATE_LIMIT` | API rate limit | `100` |
| `DLT_PIPELINE_NAME` | DLT pipeline name | `eMoney_service_pipeline_dev` |
...
**Staging:**
- Database: `eMoney_service_data_stage`
- Port: `5709`
- Production-like settings
- Staging-specific logging

**Production:**
- Database: `eMoney_service_data_prod`
- Port: `3709`
...
docker-compose exec postgres_dev psql -U postgres -d eMoney_service_data_dev

# View extracted data
SELECT * FROM eMoney_service_dev.main_data LIMIT 10;
```

### Development Tools (Dev Profile)
- **pgAdmin**: http://localhost:8080 (admin@eMoney-service.dev / admin123)
...
curl http://localhost:3709/health
```

### Scaling
```bash
# Scale production service
docker-compose --profile prod up -d --scale eMoney_service_prod=3
```

### Monitoring
```bash
# View logs
docker-compose logs -f eMoney_service_dev
...
```bash
# Check logs
docker-compose logs eMoney_service_dev

# Restart service
docker-compose restart eMoney_service_dev
```
...
**Emoney-Service API issues:**
- Verify API token in `.env` file
- Check Emoney-Service API rate limits
...
Generated by DLT Generator - Customized for Emoney-Service data extraction