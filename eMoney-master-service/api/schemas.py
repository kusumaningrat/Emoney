"""
Marshmallow schemas for eMoney Service API
...
class AuthSchema(Schema):
    """Authentication schema for eMoney service - OAuth 2.0 with JWT"""
...
class FiltersSchema(Schema):
    """Filters schema for eMoney service extraction"""
...
class ScanConfigSchema(Schema):
    """Scan configuration schema for eMoney service entity extraction"""
    scanId = fields.Str(
        required=True,
        validate=[
    ...
    type = fields.List(
        ...
        error_messages={
            ...
            'validator_failed': 'Type must contain at least one valid entity type (user, role, permission, office, logon, sharingrule, plan, goal, scenario, cashflow, networth, account, accounttype, liability, asset, assetclass, client, contact, household, spouse, relationship)'
        }
    )
    auth = fields.Nested(
        AuthSchema,
...
class PaginationSchema(Schema):
    """Pagination parameters schema - eMoney service uses page & pageSize"""
    page = fields.Int(
...
class TableQuerySchema(Schema):
    """Schema for querying specific eMoney service tables"""
    tableName = fields.Str(
        ...
        default='user',
        error_messages={
            'validator_failed': 'Invalid table name. Must be one of: user, role, permission, office, logon, sharingrule, plan, goal, scenario, cashflow, networth, account, accounttype, liability, asset, assetclass, client, contact, household, spouse, relationship'
        }
    )
...
def get_valid_entity_types() -> list:
    ...
    return ['user', 'role', 'permission', 'office', 'logon', 'sharingrule', 'plan', 'goal', 'scenario', 'cashflow', 'networth']