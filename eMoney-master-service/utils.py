...
def build_dataset_name(organization_id: str, prefix: str = "emoney_service") -> str:
...
def build_dlt_env_vars(config: Dict[str, Any]) -> Dict[str, str]:
    ...
    return {
        'DESTINATION__POSTGRES__CREDENTIALS__DATABASE': config.get('db_name', 'emoney_service_data'),
...
def create_isolated_pipeline(scan_id: str, organization_id: str, config: Dict[str, Any]):
    ...
    # Use the service prefix from config or fallback to eMoney_service
    service_prefix = config.get('service_type', 'emoney_service')
