...
def extract_users(
    ...
    """
    Extract User records from eMoney Service API
    ...
    config = get_config()
    USER_CHECKPOINT_FREQUENCY = config.USER_CHECKPOINT_FREQUENCY
    USER_PAUSE_CHECK_FREQUENCY = config.USER_PAUSE_CHECK_FREQUENCY
    USER_CANCEL_CHECK_FREQUENCY = config.USER_CANCEL_CHECK_FREQUENCY
    USER_MAX_BATCHES = config.USER_MAX_BATCHES
    ...
    logger.info("=" * 60)
    logger.info(f"Extracting Users for org: {organization_id}")
    logger.info("=" * 60)
    ...
    def should_check_cancelled():
        return batch_counter % USER_CANCEL_CHECK_FREQUENCY == 0

    def should_check_paused():
        return batch_counter % USER_PAUSE_CHECK_FREQUENCY == 0
    
    def should_save_checkpoint():
        return batch_counter % USER_CHECKPOINT_FREQUENCY == 0
    ...
    while True:
        ...
        if batch_counter > USER_MAX_BATCHES:
            logger.warning(f"Extraction of {entity} reached maximum batch limit ({USER_MAX_BATCHES})")
        ...
        try:
            ...
            for user in users:
                ...
                try:
                    ...
                    for meta_key, meta_value in extraction_metadata.items():
                        user_record[meta_key] = meta_value
                    ...
                    logger.error(f"Error processing user record: {record_err}")
            ...
            logger.info(f"Processed {batch_size} users (total: {total_records}) - batch {batch_counter}/{USER_MAX_BATCHES}")
            ...
            logger.error(f"Error extracting users at page {page} - batch {batch_counter}: {e}")
            save_checkpoint(status="error")
            for user_record in users_data:
                yield user_record
            raise

    logger.info(f"✓ Users extraction complete: {total_records} records in {batch_counter} batches")
