...
def extract_assetclass(
    ...
    """
    Extract asset classes from eMoney Service API
    
    Uses page-based pagination (page/pageSize) instead of offset-based (skip/limit)
    
    Yields asset class records with normalized field names,
    ...
    config = get_config()
    ASSETCLASS_CHECKPOINT_FREQUENCY = config.ASSETCLASS_CHECKPOINT_FREQUENCY
    ASSETCLASS_PAUSE_CHECK_FREQUENCY = config.ASSETCLASS_PAUSE_CHECK_FREQUENCY
    ASSETCLASS_CANCEL_CHECK_FREQUENCY = config.ASSETCLASS_CANCEL_CHECK_FREQUENCY
    ASSETCLASS_MAX_BATCHES = config.ASSETCLASS_MAX_BATCHES
    ...
    logger.info("=" * 60)
    logger.info(f"Extracting Asset Classes for org: {organization_id}")
    logger.info("=" * 60)
    ...
    def should_check_cancelled():
        return batch_counter % ASSETCLASS_CANCEL_CHECK_FREQUENCY == 0

    def should_check_paused():
        return batch_counter % ASSETCLASS_PAUSE_CHECK_FREQUENCY == 0
    
    def should_save_checkpoint():
        return batch_counter % ASSETCLASS_CHECKPOINT_FREQUENCY == 0
    ...
    while True:
        ...
        if batch_counter > ASSETCLASS_MAX_BATCHES:
            logger.warning(f"Extraction of {entity} reached maximum batch limit ({ASSETCLASS_MAX_BATCHES})")
        ...
        try:
            logger.info(f"Fetching asset classes (page: {page}, pageSize: {page_size}) - batch {batch_counter}/{ASSETCLASS_MAX_BATCHES}...")
            
            # Call eMoney API with page/page_size
            response = api_service.get_asset_classes(
                page=page,
                page_size=page_size,
                filters=api_filters
            )
            ...
            if isinstance(response, dict):
                asset_classes = (response.get('assetClasses') or 
                          response.get('asset_classes') or 
                          response.get('items') or 
                          response.get('data') or 
                          response.get('results') or 
                          response.get('Data') or [])
            else:
                asset_classes = response if isinstance(response, list) else []
            
            if not asset_classes:
                logger.info("No more asset classes found")
                break
            ...
            for asset_class in asset_classes:
                ...
                if test_mode and batch_size % 5 == 0:
                    if check_cancelled_callback and check_cancelled_callback():
                        ...
                        save_checkpoint(status="cancelled")
                        for record in asset_classes_data:
                            yield record
                        return
                ...
                try:
                    # Create normalized asset class record (matching API response)
                    asset_class_record = {
                        # Basic identification
                        'AssetClassID': asset_class.get('AssetClassID'),
                        
                        # Class information
                        'ClassName': asset_class.get('ClassName'),
                        'Category': asset_class.get('Category'),
                        'Description': asset_class.get('Description'),
                        'RiskLevel': asset_class.get('RiskLevel'),
                        
                        # Status
                        'Status': asset_class.get('Status'),
                        
                        # Dates
                        'CreatedDate': asset_class.get('CreatedDate'),
                    }
                    
                    # Handle nested objects with JSON serialization (if any exist in response)
                    for json_field in ['CustomFields']:
                        if asset_class.get(json_field):
                            value = asset_class.get(json_field)
                            if isinstance(value, (dict, list)):
                                asset_class_record[json_field] = json.dumps(value)
                            else:
                                asset_class_record[json_field] = value

                    # Add extraction metadata
                    for meta_key, meta_value in extraction_metadata.items():
                        asset_class_record[meta_key] = meta_value
                    
                    asset_classes_data.append(asset_class_record)
                    total_records += 1
                    batch_size += 1
                    
                except Exception as record_err:
                    logger.error(f"Error processing asset class record: {record_err}")
                    continue
            ...
            logger.info(f"Processed {batch_size} asset classes (total: {total_records}) - batch {batch_counter}/{ASSETCLASS_MAX_BATCHES}")
            
            if should_save_checkpoint():
                save_checkpoint()
            
            # Check if we got fewer records than requested (end of data)
            if len(asset_classes) < page_size:
            ...
            logger.error(f"Error extracting asset classes at page {page} - batch {batch_counter}: {e}")
            save_checkpoint(status="error")
            for record in asset_classes_data:
                yield record
            raise
    
    logger.info(f"✓ Asset Classes extraction complete: {total_records} records in {batch_counter} batches")
