...
def setup_loki_logging():
    ...
    # **FIX: Check if logging is disabled**
    if not getattr(config, 'LOKI_ENABLED', True):
        # Set up minimal logging - only CRITICAL level to suppress most logs
...
def get_logger(name):
    ...
    # **FIX: Check LOKI_ENABLED before setting up logging**
    if not getattr(config, 'LOKI_ENABLED', True):
        # Return a minimal logger that only logs CRITICAL messages
...
def log_performance(operation_name):
    ...
    def decorator(func):
        def wrapper(*args, **kwargs):
            config = get_config()
            # **FIX: Skip performance logging if disabled**
            if not getattr(config, 'LOKI_ENABLED', True):
...
def configure_app_logging(app):
    ...
    # **FIX: Check if logging is enabled before setting up Flask hooks**
    if not getattr(config, 'LOKI_ENABLED', True):
        return app.logger
