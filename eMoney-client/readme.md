# Emoney-Client Data Extraction Service

A robust Flask-RESTX API service for extracting Emoney-Client data using DLT (Data Load Tool) and PostgreSQL. Features comprehensive Swagger documentation, Docker support, and production-ready deployment,

## Features

- **Emoney-Client API Integration**: Extracts data via Emoney-Client REST API
...
│   └── api_service.py          # Emoney-Client API client
│
├── models/                     # Data models
│   └── __init__.py
│
...
- Emoney-Client API Access Token/Credentials
...
2. **Set up environment variables**:
   ...
   ```bash
   cp .env.example .env
   # Edit .env with your Emoney-Client API credentials
   ```
...
4. **Verify the setup**:
   ...
   curl http://localhost:4731/health

   # View Swagger documentation
   open http://localhost:4731/docs/
...
2. **Set up environment variables**:
   ...
   export EMONEY-CLIENT_API_TOKEN="your-token-here"
   ```

3. **Start PostgreSQL** (using Docker):

   ```bash
   docker run -d --name emoney_client_postgres \
     -e POSTGRES_DB=emoney_client_data_dev \
...
Access the interactive API documentation at: **http://localhost:4731/docs/**
...
```bash
curl -X POST http://localhost:4731/api/v1/scan/start \
  -H "Content-Type: application/json" \
  -d '{
    "config": {
      "scanId": "emoney-client-scan-001",
      "organizationId": "org-12345",
      "type": ["data"],
      "auth": {
        "accessToken": "your-emoney-client-token-here"
      },
      "filters": {
...
```bash
curl http://localhost:4731/api/v1/scan/emoney-client-scan-001/status
```

#### List All Scans

```bash
curl "http://localhost:4731/api/v1/scan/list?organizationId=org-12345&limit=10"
```
...
| `DB_NAME`                      | Database name           | `emoney_client_data_dev`     |
| `DB_USER`                      | Database user           | `postgres`                   |
| `DB_PASSWORD`                  | Database password       | `password123`                |
| `DB_SCHEMA`                    | Database schema         | `emoney_client_dev`          |
| `EMONEY-CLIENT_API_TOKEN`      | Emoney-Client API token | `""`                         |
| `EMONEY-CLIENT_API_TIMEOUT`    | API timeout seconds     | `30`                         |
| `EMONEY-CLIENT_API_RATE_LIMIT` | API rate limit          | `100`                        |
| `DLT_PIPELINE_NAME`            | DLT pipeline name       | `emoney_client_pipeline_dev` |
...
- Database: `emoney_client_data_dev`
- Port: `4731`
- Debug mode enabled
- Hot reloading
...
- Database: `emoney_client_data_stage`
- Port: `5731`
...
- Database: `emoney_client_data_prod`
- Port: `3731`
...
docker-compose exec postgres_dev psql -U postgres -d emoney_client_data_dev

# View extracted data
SELECT * FROM emoney_client_dev.main_data LIMIT 10;
```

### Development Tools (Dev Profile)

- **pgAdmin**: http://localhost:8080 (admin@emoney-client.dev / admin123)
...
curl http://localhost:3731/health
```
...
docker-compose --profile prod up -d --scale emoney_client_service_prod=3
```
...
```bash
# View logs
docker-compose logs -f emoney_client_service_dev
...
```bash
# Check logs
docker-compose logs emoney_client_service_dev

# Restart service
docker-compose restart emoney_client_service_dev
```
...
**Emoney-Client API issues:**

- Verify API token in `.env` file
- Check Emoney-Client API rate limits
...
Generated by DLT Generator - Customized for Emoney-Client data extraction

<!-- ci: test dev deploy gate -->
