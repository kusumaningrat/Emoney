"""
Marshmallow schemas for eMoney Account Service API
...
class AuthSchema(Schema):
    """Authentication schema for eMoney Account Service - OAuth 2.0 with JWT"""
...
class FiltersSchema(Schema):
    """Filters schema for eMoney Account Service extraction"""
...
class TableQuerySchema(Schema):
    ...
    tableName = fields.Str(
        ...
        default='account',
        error_messages={
            'validator_failed': 'Invalid table name. Must be one of: account, accounttype, liability, asset, assetclass'
        }
    )
