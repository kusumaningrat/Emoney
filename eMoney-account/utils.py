...
def build_dataset_name(organization_id: str, prefix: str = "emoney_account") -> str:
...
def build_dlt_env_vars(config: Dict[str, Any]) -> Dict[str, str]:
    ...
    return {
        'DESTINATION__POSTGRES__CREDENTIALS__DATABASE': config.get('db_name', 'emoney_account_data'),
...
def build_sql_queries(dataset_name: str, table_name: str, limit: int = 100, offset: int = 0) -> Dict[str, str]:
    ...
    # Note: DLT converts PascalCase to snake_case, so AccountID becomes account_id
    primary_key_map = {
        'account': 'account_id',
    ...
    # Get the primary key for this table, fallback to _dlt_id if not found
    primary_key = primary_key_map.get(table_name.lower(), '_dlt_id')
    
    return {
        'count': f"SELECT COUNT(*) as total FROM {full_table_name}",
        'data': f"""
            SELECT * FROM {full_table_name}
            ORDER BY "_extracted_at" DESC, "{primary_key}"
...
def create_isolated_pipeline(scan_id: str, organization_id: str, config: Dict[str, Any]):
    ...
    service_prefix = config.get('service_type', 'emoney_account_service')
