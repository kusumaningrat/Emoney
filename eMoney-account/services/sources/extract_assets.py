...
def extract_assets(
    ...
    """
    Extract assets from eMoney Advisor API
    
    Uses page-based pagination (page/pageSize) instead of offset-based (skip/limit)
    
    Yields asset records with normalized field names,
    ...
    config = get_config()
    ASSET_CHECKPOINT_FREQUENCY = config.ASSET_CHECKPOINT_FREQUENCY
    ASSET_PAUSE_CHECK_FREQUENCY = config.ASSET_PAUSE_CHECK_FREQUENCY
    ASSET_CANCEL_CHECK_FREQUENCY = config.ASSET_CANCEL_CHECK_FREQUENCY
    ASSET_MAX_BATCHES = config.ASSET_MAX_BATCHES
    ...
    logger.info("=" * 60)
    logger.info(f"Extracting Assets for org: {organization_id}")
    logger.info("=" * 60)
    ...
    if filters:
        ...
        if 'asset_class' in filters:
            api_filters['asset_class'] = filters['asset_class']
    ...
    def should_check_cancelled():
        return batch_counter % ASSET_CANCEL_CHECK_FREQUENCY == 0

    def should_check_paused():
        return batch_counter % ASSET_PAUSE_CHECK_FREQUENCY == 0
    
    def should_save_checkpoint():
        return batch_counter % ASSET_CHECKPOINT_FREQUENCY == 0
    ...
    while True:
        ...
        if batch_counter > ASSET_MAX_BATCHES:
            logger.warning(f"Extraction of {entity} reached maximum batch limit ({ASSET_MAX_BATCHES})")
        ...
        try:
            logger.info(f"Fetching assets (page: {page}, pageSize: {page_size}) - batch {batch_counter}/{ASSET_MAX_BATCHES}...")
            
            # Call eMoney API with page/page_size
            response = api_service.get_assets(
                page=page,
                page_size=page_size,
                filters=api_filters,
            ...
            for asset in assets:
                ...
                try:
                    # Create normalized asset record (matching API response)
                    asset_record = {
                        # Basic identification
                        'AssetID': asset.get('AssetID'),
                        'AccountID': asset.get('AccountID'),
                        'AssetClassID': asset.get('AssetClassID'),
                        
                        # Asset information
                        'SecurityName': asset.get('SecurityName'),
                        ...
                        'CostBasis': asset.get('CostBasis'),
                        
                        # Performance
                        'UnrealizedGain': asset.get('UnrealizedGain'),
                        'UnrealizedGainPercent': asset.get('UnrealizedGainPercent'),
                        
                        # Status
                        'Status': asset.get('Status'),
                        
                        # Dates
                        'AsOfDate': asset.get('AsOfDate'),
                        'CreatedDate': asset.get('CreatedDate'),
                        'ModifiedDate': asset.get('ModifiedDate'),
                    }
                    
                    # Handle nested objects with JSON serialization (if any exist in response)
                    for json_field in ['Transactions', 'CustomFields']:
                        if asset.get(json_field):
                            value = asset.get(json_field)
                            if isinstance(value, (dict, list)):
                                asset_record[json_field] = json.dumps(value)
                            else:
                                asset_record[json_field] = value

                    # Add extraction metadata
                    for meta_key, meta_value in extraction_metadata.items():
                        asset_record[meta_key] = meta_value
                    ...
                    logger.error(f"Error processing asset record: {record_err}")
                    continue
            ...
            logger.info(f"Processed {batch_size} assets (total: {total_records}) - batch {batch_counter}/{ASSET_MAX_BATCHES}")
            ...
            logger.error(f"Error extracting assets at page {page} - batch {batch_counter}: {e}")
            save_checkpoint(status="error")
            for record in assets_data:
                yield record
            raise
    
    logger.info(f"✓ Assets extraction complete: {total_records} records in {batch_counter} batches")
