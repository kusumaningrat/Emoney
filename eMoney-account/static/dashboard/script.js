...
document.addEventListener('DOMContentLoaded', function () {
    ...
    header.innerHTML = `
        <div class="header-title">
            <h1><i class="data-icon"></i>eMoney Account Extraction Dashboard</h1>
            <span class="subtitle">Monitor and manage your extraction tasks</span>
        </div>
    ...
    document.getElementById('refresh-btn').addEventListener('click', function () {
        ...
        if (prevBtn && !prevBtn.disabled) {
            ...
            if (nextBtn) {
                offset = parseInt(prevBtn.getAttribute('data-offset'), 10) + parseInt(prevBtn.getAttribute('data-limit'), 10);
            }
        }
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    if (headerControls) {
        ...
        startScanBtn.innerHTML = '<i class="plus-icon"></i> Start Scan';
...
function fetchTasks(offset = 0, limit = 10) {
    document.getElementById('task-list').innerHTML = '<div class="loading-indicator"><div class="spinner"></div><p>Loading tasks...</p></div>';
        ...
        .then(data => {
            if (data.success) {
                ...
                document.getElementById('task-list').innerHTML =
                    `<div class="error-message"><p>Error loading tasks: ${data.message || 'Unknown error'}</p></div>`;
            }
        })
        .catch(error => {
            console.error('Error fetching tasks:', error);
            document.getElementById('task-list').innerHTML =
                `<div class="error-message"><p>Error loading tasks: ${error.message || 'Network error'}</p></div>`;
        });
}
...
function displayTaskStats(tasks) {
    ...
    tasks.forEach(task => {
        ...
        else if (task.status === 'failed' || task.status === 'crashed') stats.failed++;
        else if (task.status === 'pending' || task.status === 'paused') stats.pending++;
    });
...
function displayTasks(tasks, pagination) {
    if (!tasks.length) {
        document.getElementById('task-list').innerHTML = '<div class="no-data-message"><p>No tasks found.</p></div>';
        return;
    }
    ...
    <div class="results-header">
        <h4>Data Extraction Tasks</h4>
        <p class="pagination-info">
            Showing ${pagination.offset + 1} to ${pagination.offset + pagination.returned} 
    ...
    tasks.forEach(task => {
        ...
        const lastHeartbeat = task.lastHeartbeat ? new Date(task.lastHeartbeat).toLocaleString() : 'N/A';
        const taskType = task.type || (task.config && task.config.type ?
            (Array.isArray(task.config.type) ? task.config.type.join(', ') : task.config.type) : 'N/A');
    ...
    html += `
        <div class="pagination-controls">
            <button 
                ...
                data-offset="${Math.max(0, pagination.offset - pagination.limit)}"
...
function showTaskDetails(task) {
    document.getElementById('modal-title').textContent = `Task Details: ${task.scanId}`;

    document.getElementById('stream-tab').style.display = (task.status === 'completed') ? 'block' : 'none';
    document.getElementById('results-tab').style.display = task.status === 'completed' ? 'block' : 'none';
    ...
    let detailsHtml = `
        <div>
            ...
            <p><strong>Organization ID:</strong> ${task.organizationId || 'N/A'}</p>
            <p><strong>Start Time:</strong> ${startTime}</p>
            <p><strong>End Time:</strong> ${endTime}</p>
            <p><strong>Records Extracted:</strong> ${task.recordsExtracted || 0}</p>
...
function displayConfigDetails() {
    ...
    if (config.type) {
        const types = Array.isArray(config.type) ? config.type.join(', ') : config.type;
        html += `<p><strong>Type:</strong> ${types}</p>`;
    }
    ...
    if (config.auth) {
        ...
        if (config.auth.apiKey) {
            ...
            html += `Username/Password (secured)`;
        } else {
    ...
    if (config.filters) {
        ...
        if (config.filters.includeArchived !== undefined) {
            html += `<li>Include Archived: ${config.filters.includeArchived ? 'Yes' : 'No'}</li>`;
        }

        if (config.filters.dateRange) {
            const dateRange = config.filters.dateRange;
            html += `<li>Date Range: ${dateRange.startDate || 'N/A'} to ${dateRange.endDate || 'N/A'}</li>`;
        }

        if (config.filters.properties && Array.isArray(config.filters.properties)) {
            html += `<li>Properties: ${config.filters.properties.join(', ')}</li>`;
        }
...
function fetchTableData(taskId, offset = 0, limit = 10) {
    if (!currentTask || !currentTask.config || !currentTask.config.type) {
        document.getElementById('results').innerHTML = '<p>Error: Cannot determine task type</p>';
        return;
    }
    ...
    document.getElementById('results').innerHTML = '<div class="loading-indicator"><div class="spinner"></div><p>Loading data...</p></div>';

    fetch(`/api/results/${taskId}/result?tableName=${tableName}&offset=${offset}&limit=${limit}`)
        .then(response => response.json())
        .then(data => {
            if (data.success) {
                ...
                document.getElementById('results').innerHTML =
                    `<div class="error-message"><p>Error loading data: ${data.message || 'Unknown error'}</p></div>`;
            }
        })
        .catch(error => {
            console.error('Error fetching table data:', error);
            document.getElementById('results').innerHTML =
                `<div class="error-message"><p>Error loading data: ${error.message || 'Network error'}</p></div>`;
        });
}
...
function displayTableData(data, currentOffset, limit) {
    if (!data || !data.records || !data.records.length) {
        document.getElementById('results').innerHTML = '<div class="no-data-message"><p>No records available for this scan type.</p></div>';
        return;
    }
    ...
    let html = `
        <div class="results-header">
            ...
            <p class="pagination-info">
                Showing ${pagination.offset + 1} to ${Math.min(pagination.offset + data.records.length, pagination.total)} 
...
function startStreaming() {
    ...
    streamContainer.innerHTML = '<div class="loading-indicator"><div class="spinner"></div><p>Loading stream data...</p></div>';
    ...
    fetch(`/api/stream/${currentTask.scanId}?offset=0&limit=100`)
            ...
            if (data.success) {
                ...
                streamContainer.innerHTML = `
                    <div class="error-message">
                        <p>Error loading stream data: ${data.message || 'Unknown error'}</p>
                    </div>
                `;
                resetStreamButton();
            }
        ...
        .catch(error => {
            ...
            streamContainer.innerHTML = `
                <div class="error-message">
                    <p>Error loading stream data: ${error.message || 'Network error'}</p>
                </div>
            `;
            resetStreamButton();
        });
...
function displayStatsFromAPI(data) {
    ...
    let statsHtml = `
        <div class="task-stats">
            ...
            <div class="stat-card completed">
                <span class="stat-value">${statusBreakdown.completed || 0}</span>
                <span class="stat-label">Completed</span>
            </div>
            <div class="stat-card failed">
                <span class="stat-value">${(statusBreakdown.failed || 0) + (statusBreakdown.crashed || 0)}</span>
                <span class="stat-label">Failed</span>
            </div>
        </div>
        <div class="service-info">
            ...
            <div class="extra-stats">
                <div class="extra-stat-item">
                    ...
                    <span class="extra-stat-value">${serviceInfo.name || 'N/A'}</span>
                </div>
                <div class="extra-stat-item">
                    <span class="extra-stat-label">Source Type</span>
                    <span class="extra-stat-value">${serviceInfo.source_type || 'N/A'}</span>
                </div>
                <div class="extra-stat-item">
                    <span class="extra-stat-label">Total Records Extracted</span>
                    <span class="extra-stat-value">${jobStats.total_records_extracted?.toLocaleString() || 0}</span>
                </div>
                <div class="extra-stat-item">
                    <span class="extra-stat-label">Recent Jobs (7 Days)</span>
                    <span class="extra-stat-value">${jobStats.recent_jobs_7_days || 0}</span>
                </div>
                <div class="extra-stat-item">
                    <span class="extra-stat-label">Database</span>
                    <span class="extra-stat-value">${data.database?.database || 'N/A'}</span>
                </div>
                <div class="extra-stat-item">
                    <span class="extra-stat-label">Database Size</span>
                    <span class="extra-stat-value">${data.database?.size_pretty || 'N/A'}</span>
                </div>
                <div class="extra-stat-item">
                    <span class="extra-stat-label">Database Status</span>
                    <span class="extra-stat-value ${data.database?.connected ? 'status-completed' : 'status-failed'}">
...
function createStartScanModal() {
    ...
    modal.innerHTML = `
        <div class="modal-content">
            ...
            <h2>Start New Data Extraction Scan</h2>
                ...
                <form id="start-scan-form">
                    <div class="form-section">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" id="scanId" name="scanId" value="emoney-account-scan-${currentYear}-001" placeholder="e.g., emoney-account-scan-2025-001" required>
                            <span class="form-hint">Unique identifier for this scan</span>
                        </div>
                        
                        <div class="form-group">
                            <label for="organizationId">Organization ID</label>
                            <input type="text" id="organizationId" name="organizationId" value="org-12345" placeholder="e.g., org-12345" required>
                            <span class="form-hint">ID of the organization to scan</span>
                        </div>
                    </div>
                    ...
                    <div class="form-section">
                        ...
                        <div class="form-group radio-group">
                            <p class="radio-label">Select the type of data to extract:</p>
                    ...
                    <div class="form-actions">
                        ...
                        <button type="submit" class="submit-action-btn">Start Scan</button>
                    </div>
                </form>
            </div>
        </div>
    ...
    document.getElementById('close-scan-modal').addEventListener('click', closeStartScanModal);
    document.getElementById('cancel-scan-btn').addEventListener('click', closeStartScanModal);
    document.getElementById('start-scan-form').addEventListener('submit', submitScanForm);
}
...
function submitScanForm(event) {
    ...
    fetch('/api/scan/start', {
        ...
        .catch(error => {
            ...
            alert(`Error starting scan: ${error.message || 'Network error'}`);
        });
}