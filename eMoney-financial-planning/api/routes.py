"""
Flask-RESTX route definitions for Financial Planning Core Data Extraction API
"""
import asyncio
...
from .schemas import (
    ...
    validate_limit_offset_params,  # Changed from validate_pagination_params
...
def create_api():
    ...
    authorizations = {
        'hmac_auth': {
            'type': 'apiKey',
            'in': 'header',
            'name': 'X-Financial-Planning-Signature',
            'description': 'HMAC signature for request authentication'
        },
        'hmac_timestamp': {
            'type': 'apiKey',
            'in': 'header',
            'name': 'X-Financial-Planning-Timestamp',
            'description': 'Unix timestamp for request freshness'
        },
        'hmac_client_id': {
            'type': 'apiKey',
            'in': 'header',
            'name': 'X-Financial-Planning-Client-ID',
    ...
    # Create namespaces
    scan_ns = Namespace('scan', description='Financial planning scan operations')
    ...
    class StartScan(Resource):
        ...
        @hmac_auth_required
        def post(self):
            """Start a new financial planning data extraction scan"""
            request_id = getattr(g, 'request_id', str(uuid.uuid4()))
            
            try:
                logger.info(
                    "Starting financial planning scan request",
                    extra={
                ...
                logger.info(
                    "Financial planning scan accepted for processing",
                    extra={
                        'request_id': request_id,
                        'scan_id': scan_config.scanId,
                ...
                log_business_event(
                    logger,
                    "financial_planning_scan_creation_accepted",
                    scan_id=scan_config.scanId,
                    organization_id=scan_config.organizationId,
                    entity_types=scan_config.type
                )
                ...
                return {
                    "success": True,
                    "message": "Financial planning scan initialization accepted and is now processing in the background."
                }, 202
    ...
    class ScanStatus(Resource):
        ...
        def get(self, scan_id):
            """Get the status of a specific financial planning scan"""
            request_id = getattr(g, 'request_id', str(uuid.uuid4()))
        ...
        def post(self, scan_id):
            ...
            try:
                ...
                if result['success']:
                    ...
                    log_business_event(logger, "financial_planning_scan_cancelled", scan_id=scan_id)
                    return result
                else:
                    logger.warning(
    ...
    class PauseScan(Resource):
        ...
        def post(self, scan_id):
            ...
            try:
                ...
                if result['success']:
                    logger.info(
                        "Financial planning scan paused successfully",
                        extra={'request_id': request_id, 'scan_id': scan_id}
                    )
                    log_business_event(logger, "financial_planning_scan_paused", scan_id=scan_id)
                    return result
                else:
    ...
    class ResumeScan(Resource):
        ...
        def post(self, scan_id):
            ...
            try:
                ...
                if result['success']:
                    logger.info(
                        "Financial planning scan resumed successfully",
                        extra={'request_id': request_id, 'scan_id': scan_id}
                    )
                    log_business_event(logger, "financial_planning_scan_resumed", scan_id=scan_id)
                    return result
                else:
                    status_code = 404 if "not found" in result['message'].lower() else 400
                    logger.warning(
    ...
    class ListScans(Resource):
        ...
        @hmac_auth_required
        def get(self):
            """List all financial planning scans with optional filtering and pagination"""
            request_id = getattr(g, 'request_id', str(uuid.uuid4()))
            
            try:
                organization_id = request.args.get('organizationId')
                
                # Validate pagination - UPDATED FUNCTION NAME
                try:
    ...
    class RemoveScan(Resource):
        ...
        def delete(self, scan_id):
            """Remove a financial planning scan and its data"""
            request_id = getattr(g, 'request_id', str(uuid.uuid4()))
            ...
            try:
                ...
                if result['success']:
                    logger.info(
                        "Financial planning scan removed",
                        extra={'request_id': request_id, 'scan_id': scan_id}
                    )
                    log_business_event(logger, "financial_planning_scan_removed", scan_id=scan_id)
                    return result
                else:
                    logger.error(
                        "Scan removal failed",
    ...
    class GetScanResults(Resource):
        @results_ns.param('tableName', 'Name of the table to query (e.g., plan, goal, scenario, cashflow, networth)', default='plan')
        ...
        @hmac_auth_required
        def get(self, scan_id):
            """Get financial planning scan results with pagination and table selection"""
            request_id = getattr(g, 'request_id', str(uuid.uuid4()))
            
            try:
                table_name = request.args.get('tableName', 'plan')
                ...
                if result['success']:
                    logger.debug(
                        "Financial planning results retrieved",
                        extra={
                            'request_id': request_id,
                            'scan_id': scan_id,
    ...
    class StreamScanData(Resource):
        ...
        def get(self, scan_id):
            """Stream financial planning scan data to Kafka"""
    ...
    class PipelineInfo(Resource):
        @hmac_auth_required
        def get(self):
            """Get information about the financial planning DLT pipeline configuration"""
            request_id = getattr(g, 'request_id', str(uuid.uuid4()))
    ...
    class Cleanup(Resource):
        ...
        @hmac_auth_required
        def post(self):
            """Clean up old financial planning scan results"""
            request_id = getattr(g, 'request_id', str(uuid.uuid4()))
            ...
            try:
                ...
                log_security_event(
                    logger,
                    "financial_planning_data_cleanup_performed",
                    cleaned_count=cleaned_count,
                    days_old=days_old
                )
    ...
    class DetectCrashedJobs(Resource):
        ...
        @hmac_auth_required
        def post(self):
            """Detect and mark crashed financial planning extraction jobs"""
            request_id = getattr(g, 'request_id', str(uuid.uuid4()))
    ...
    class ServiceStats(Resource):
        @hmac_auth_required
        def get(self):
            """Get financial planning extraction service statistics"""
            request_id = getattr(g, 'request_id', str(uuid.uuid4()))
    ...
    class Health(Resource):
        def get(self):
            """Health check endpoint for financial planning extraction service"""
            request_id = getattr(g, 'request_id', str(uuid.uuid4()))
    ...
    logger.info(
        "Financial Planning Core API created successfully",
        extra={
            'operation': 'api_creation',
            'service': config.DLT_PIPELINE_NAME,
            'entities': ['FinancialPlan', 'Goal', 'Scenario', 'CashFlow', 'NetWorth']
        }
    )
