...
class ExtractionService:
    """Service for orchestrating financial planning data extraction pipelines using DLT"""

    def __init__(self, config: Dict[str, Any], source_type: str = "financial_planning"):
        self.config = config
        ...
        self.logger.debug(
            "Financial planning extraction service initialized",
            extra={
                "operation": "service_init",
                "source_type": source_type,
                "pipeline_name": self.pipeline_name,
                "entities": ["Plan", "Goal", "Scenario", "CashFlow", "NetWorth"]
            },
        )
    ...
    def _setup_dlt_environment(self):
        ...
        try:
            ...
            self.logger.debug(
                "DLT environment configured for financial planning",
    ...
    def create_source_with_checkpoints(
        ...
        """Create DLT data source with checkpointing support for financial planning entities"""
        try:
            ...
            if latest_checkpoint:
                ...
                # Normalize entity_type to lowercase and remove spaces/underscores
                entity_type = (
                    entity_type.lower().replace(" ", "").replace("_", "")
                    if entity_type
                    else "financialplan"
                )
                ...
                if latest_checkpoint.get("checkpoint_data"):
                    ...
                    self.logger.info(
                        "Resuming financial planning extraction from checkpoint",
                        extra={
            ...
            def checkpoint_callback(job_id_param: str, entity_data: Dict[str, Any]):
                """Save checkpoint during financial planning data extraction"""
                try:
                    if not entity_data:
                        return

                    # entity_data structure: {'entity': 'financialplan', 'records_processed': 100, 'checkpoint_data': {...}}
                    checkpoint_data = {
            ...
            def check_paused_callback():
                """Check if the financial planning extraction job has been paused"""
                job_status = self.job_service.get_job_status(job_id)
                if job_status and job_status.get("status") == JobStatus.PAUSED.value:
                    self.logger.info(
                        "Job pause detected during financial planning extraction",
            ...
            def check_cancelled_callback():
                """Check if the financial planning extraction job has been cancelled"""
                job_status = self.job_service.get_job_status(job_id)
                if job_status and job_status.get("status") == JobStatus.CANCELLED.value:
                    self.logger.info(
                        "Job cancellation detected during financial planning extraction",
                        extra={"operation": "create_source", "job_id": job_id},
                    )
                    return True
                return False
            ...
            self.logger.error(
                "Failed to create financial planning data source",
                extra={
                    "operation": "create_source",
                    "job_id": job_id,
                    "error": str(e),
    ...
    async def start_scan(self, request_config: Dict[str, Any]) -> Dict[str, Any]:
        """Start new financial planning data extraction scan"""
        scan_id = request_config.get("scanId", "unknown")

        try:
            self.logger.info(
                "Financial planning scan initiation requested",
                extra={
                    "operation": "start_scan",
                    "scan_id": scan_id,
                    "organization_id": request_config.get("organizationId"),
                    "entity_types": request_config.get("type", []),
                },
            )
            ...
            log_business_event(
                self.logger,
                "financial_planning_scan_initiated",
                scan_id=scan_id,
                organization_id=request_config.get("organizationId"),
                entity_types=request_config.get("type", []),
            )
            ...
            self.logger.error(
                "Failed to initiate financial planning scan",
            ...
            return {
                ...
                "scanId": scan_id,
                "error": str(e),
                "message": "Failed to initiate financial planning scan",
            }

    async def _execute_scan_with_setup(self, request_config: Dict[str, Any]):
        """Handle setup validation and execution in background for financial planning scan"""
        scan_id = request_config.get("scanId", "unknown")

        try:
            self.logger.info(
                "Starting financial planning scan setup",
            ...
            if not check_database_health():
                ...
                self.logger.error(
                    "Database health check failed for financial planning scan",
                    extra={"operation": "scan_setup", "scan_id": scan_id},
                )
                try:
            ...
            if existing_job:
                if existing_job["status"] == JobStatus.CRASHED.value:
                    ...
                    self.logger.info(
                        "Resuming crashed financial planning job",
                        extra={"operation": "scan_setup", "scan_id": scan_id},
                    )
                else:
                    self.logger.warning(
                        "Financial planning job already exists",
                        extra={
                ...
                self.logger.info(
                    "Financial planning job created successfully",
                    extra={"operation": "scan_setup", "scan_id": scan_id},
                )
            ...
            self.logger.info(
                "Setup completed, starting financial planning extraction",
                extra={"operation": "scan_setup", "scan_id": scan_id},
            )
            ...
            self.logger.error(
                "Financial planning scan setup failed",
    ...
    async def _execute_scan(self, job_id: str):
        """Execute the financial planning data extraction pipeline with isolated pipeline to prevent race conditions"""
        working_dir = None
        try:
            job = self.job_service.get_job(job_id, decrypt=True)
            if not job:
                self.logger.error(
                    "Financial planning job not found for execution",
                    extra={"operation": "execute_scan", "job_id": job_id},
                )
                return

            self.logger.info(
                "Starting financial planning pipeline execution",
                extra={"operation": "execute_scan", "job_id": job_id},
            )
            ...
            self.logger.info(
                "Financial planning DLT pipeline started with isolated configuration",
                extra={
            ...
            if not current_job:
                self.logger.error(
                    "Financial planning job disappeared during execution",
                    extra={"operation": "execute_scan", "job_id": job_id},
                )
                return
            ...
            if current_status == JobStatus.CANCELLED.value:
                self.logger.info(
                    "Financial planning job was cancelled during execution, keeping cancelled status",
                    extra={"operation": "execute_scan", "job_id": job_id},
                )
                return
            ...
            if current_status == JobStatus.PAUSED.value:
                self.logger.info(
                    "Financial planning job was paused during execution, keeping paused status",
                    extra={"operation": "execute_scan", "job_id": job_id},
                )
                return
            ...
            if latest_checkpoint:
                ...
                if "paused" in checkpoint_phase.lower():
                    self.logger.info(
                        "Financial planning pipeline stopped due to pause, keeping paused status",
                        extra={
                            "operation": "execute_scan",
                            "job_id": job_id,
            ...
            if latest_checkpoint:
                ...
                records_extracted = (
                    latest_checkpoint.get("recordsProcessed") or 
                    latest_checkpoint.get("records_processed") or 
                    0
            )
            ...
            self.logger.info(
                "Financial planning scan completed successfully",
                extra={
                    "operation": "execute_scan",
                    "job_id": job_id,
            ...
            log_business_event(
                self.logger,
                "financial_planning_scan_completed",
                job_id=job_id,
                records_extracted=records_extracted,
            )
            ...
            self.logger.error(
                "Financial planning scan execution failed",
            ...
            log_business_event(
                self.logger,
                "financial_planning_scan_failed",
                job_id=job_id,
                error=str(e),
            )
    ...
    def get_scan_status(self, scan_id: str) -> Optional[Dict[str, Any]]:
        """Get financial planning scan status"""
        return self.job_service.get_job_status(scan_id)

    def cancel_scan(self, scan_id: str) -> Dict[str, Any]:
        """Cancel a running financial planning scan"""
        result = self.job_service.cancel_job(scan_id)

        if result.get("success"):
            self.logger.info(
                "Financial planning scan cancelled",
                extra={"operation": "cancel_scan", "scan_id": scan_id},
            )
            log_business_event(
                self.logger, "financial_planning_scan_cancelled", scan_id=scan_id
            )
    ...
    def list_scans(
        ...
        """List financial planning scans with optional filtering"""
        try:
            scans = self.job_service.list_jobs(organization_id, limit, offset)

            self.logger.debug(
                "Financial planning scans listed",
                extra={
            ...
            self.logger.error(
                "Error listing financial planning scans",
    ...
    def detect_crashed_jobs(self, timeout_minutes: int = 10) -> List[str]:
        """Detect crashed financial planning jobs"""
        try:
            crashed_jobs = self.job_service.detect_crashed_jobs(timeout_minutes)

            if crashed_jobs:
                self.logger.warning(
                    "Crashed financial planning jobs detected",
                    extra={
                ...
                log_security_event(
                    self.logger,
                    "crashed_financial_planning_jobs_detected",
                    severity="WARNING",
                    crashed_count=len(crashed_jobs),
                )
            ...
            self.logger.error(
                "Error detecting crashed financial planning jobs",
    ...
    def cleanup_old_scans(self, days_old: int = 7) -> int:
        ...
        try:
            ...
            log_security_event(
                self.logger,
                "financial_planning_scans_cleanup_performed",
                deleted_count=deleted_count,
                days_old=days_old,
            )
            ...
            self.logger.error(
                "Error during financial planning cleanup",
    ...
    def get_scan_statistics(
        ...
        """Get financial planning scan statistics"""
        return self.job_service.get_job_statistics(organization_id)

    def get_scan_results(
        self,
        scan_id: str,
        table_name: str = "financialplan",
        limit: int = 100,
        offset: int = 0,
    ) -> Dict[str, Any]:
        """Get financial planning scan results with pagination"""
        try:
            ...
            if result["success"]:
                ...
                self.logger.debug(
                    "Financial planning scan results retrieved",
                    extra={
            ...
            self.logger.error(
                "Error getting financial planning scan results",
                extra={
                    "operation": "get_scan_results",
                    "scan_id": scan_id,
                    "table_name": table_name,
    ...
    def get_available_tables(self, scan_id: str) -> Dict[str, Any]:
        """Get available tables for a completed financial planning scan"""
        try:
            job = self.job_service.get_job(scan_id)
            if not job:
                return {"success": False, "message": "Scan not found"}
            ...
            tables = self.db_service.get_tables_with_counts(
                dataset_name, table_info, config=self.config
            )

            self.logger.debug(
                "Available financial planning tables retrieved",
                extra={
            ...
            self.logger.error(
                "Error getting available financial planning tables",
                extra={
                    "operation": "get_available_tables",
                    "scan_id": scan_id,
                    "error": str(e),
    ...
    def get_pipeline_info(self) -> Dict[str, Any]:
        """Get DLT pipeline configuration info for financial planning"""
        try:
            ...
            self.logger.warning(
                "Financial planning pipeline info retrieval failed",
    ...
    def remove_scan(self, scan_id: str) -> Dict[str, Any]:
        """Remove financial planning scan and associated data"""
        try:
            job = self.job_service.get_job(scan_id)
            if not job:
                return {"success": False, "message": "Scan not found"}
            ...
            if removed:
                self.logger.info(
                    "Financial planning scan removed successfully",
                    extra={
                ...
                log_business_event(
                    self.logger, "financial_planning_scan_removed", scan_id=scan_id
                )

                return {
                    "success": True,
                    "message": f"Financial planning scan {scan_id} successfully removed",
                    "data": {
                        "scanId": scan_id,
            ...
            self.logger.error(
                "Error removing financial planning scan",
    ...
    def get_service_statistics(self) -> Dict[str, Any]:
        """Get comprehensive financial planning service statistics"""
        try:
            ...
            stats = {
                "service": {
                    "name": "financial_planning_extraction_service",
                    "source_type": self.source_type,
            ...
            self.logger.debug(
                "Financial planning service statistics retrieved",
                extra={
            ...
            self.logger.error(
                "Error getting financial planning service statistics",
