...
class DatabaseService:
    def __init__(self, pipeline_name: str, destination, config: Dict[str, Any] = None):
        self.pipeline_name = pipeline_name
        self.destination = destination
        self.config = config or {}
        self.logger = logging.getLogger(__name__)

    def _create_temp_pipeline_for_read(self, dataset_name: str, config: Dict[str, Any] = None):
        """Create temporary isolated pipeline for read operations"""
        temp_scan_id = str(uuid.uuid4())[:8]
        temp_org_id = "temp-org"
        
        if not config:
            # Use instance config or fallback
            config = self.config or {}
        
        return create_isolated_pipeline(temp_scan_id, temp_org_id, config)
    ...
    def get_scan_data(self, dataset_name: str, table_name: str = "users", 
                      limit: int = 100, offset: int = 0, config: Dict[str, Any] = None) -> Dict[str, Any]:
        """Get scan data with pagination using isolated pipeline"""
        working_dir = None
        try:
            # Create isolated pipeline for read operation to prevent race conditions
            pipeline, working_dir = self._create_temp_pipeline_for_read(dataset_name, config)
    ...
    def get_tables_with_counts(self, dataset_name: str, table_info: Dict[str, int] = None,
                              config: Dict[str, Any] = None) -> List[Dict[str, Any]]:
        """Get available tables with their row counts using isolated pipeline"""
        working_dir = None
        try:
            # Create isolated pipeline for read operation to prevent race conditions
            pipeline, working_dir = self._create_temp_pipeline_for_read(dataset_name, config)
    ...
    def get_database_info(self, config: Dict[str, Any] = None) -> Dict[str, Any]:
        """Get database statistics and health information using isolated pipeline"""
        working_dir = None
        try:
            # Create isolated pipeline for read operation to prevent race conditions
            pipeline, working_dir = self._create_temp_pipeline_for_read("temp_dataset", config)
    ...
    def remove_dataset_tables(self, dataset_name: str, scan_id: str, config: Dict[str, Any] = None) -> int:
        """Remove records for a specific scan from dataset tables using isolated pipeline"""
        working_dir = None
        try:
            records_removed = 0
            
            # Create isolated pipeline for write operation to prevent race conditions
            pipeline, working_dir = self._create_temp_pipeline_for_read(dataset_name, config)
