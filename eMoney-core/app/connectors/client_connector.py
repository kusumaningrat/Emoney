...
class WealthboxClientConnector:
    """
    Connector for Wealthbox Client Service data extraction operations.
    Handles client entity scanning and data extraction for contacts, clients,
    applicants, relationships, and households.
    """
    ...
    def __init__(self, api_key: Optional[str] = None, timeout: float = 5.0):
        ...
        logger.info(f"Initialized WealthboxClientConnector for {self.service_url}")
    ...
    async def start_scan(self, scan_config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            # Handle filters - only include dateRange if present
            if "filters" not in config:
                config["filters"] = {}
            else:
                # Clean filters to only include dateRange
                original_filters = config["filters"]
                cleaned_filters = {}
                
                # Only include dateRange if it exists
                if "dateRange" in original_filters:
                    cleaned_filters["dateRange"] = original_filters["dateRange"]
                
                config["filters"] = cleaned_filters
