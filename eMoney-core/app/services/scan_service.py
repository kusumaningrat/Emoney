...
class ScanService:
    ...
    async def _route_to_pipeline_service(self, scan, scan_type: str, offset: int, limit: int) -> Dict[str, Any]:
        ...
        stream_url = f"{pipeline_base_url}/api/stream/{job_id}?{params}"
        ...
        try:
            ...
            with urllib.request.urlopen(req, timeout=60) as response:
                ...
                logger.info(f"DEBUG: Pipeline response status: {response_code}")
                ...
                if response_code == 200:
                    ...
                    logger.error(f"DEBUG: Pipeline returned 400: {response_text}")
                    ...
                    logger.error(f"DEBUG: Pipeline error {response_code}: {response_text}")
                    raise Exception(f"Pipeline service error {response_code}: {response_text}")
            ...
            raise Exception(f"Invalid response from pipeline service: {str(e)}")
        except Exception as e:
            logger.error(f"Unexpected error routing to pipeline service: {str(e)}")
            raise Exception(f"Internal routing error: {str(e)}")
