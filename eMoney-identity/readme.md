...
- **Emoney-Identity API Integration**: Extracts data via Emoney-Identity REST API
...
│   └── api_service.py          # Emoney-Identity API client
│
├── models/                     # Data models
│   └── __init__.py
│
...
- Emoney-Identity API Access Token/Credentials
...
2. **Set up environment variables**:
   ...
   export EMONEY-IDENTITY_API_TOKEN="your-token-here"
   ```
...
curl -X POST http://localhost:4709/api/v1/scan/start \
  ...
  -d '{
    "config": {
      "scanId": "emoney-identity-scan-001",
      "organizationId": "org-12345",
      "type": ["data"],
      "auth": {
        "accessToken": "your-emoney-identity-token-here"
      },
      "filters": {
...
# View extracted data
SELECT * FROM emoney_identity_dev.main_data LIMIT 10;
```
...
# Restart service
docker-compose restart emoney_identity_service_dev
```
...
Generated by DLT Generator - Customized for Emoney-Identity data extraction